...
注释 | // /**/ | // /**/ | // /**/ | # ''' """ | // /**/ | # =begin =end |
函数结尾； | 无 | 有 | 有 | 无 | 无 | 无 |
一维数组 | int* result = (int *)malloc(numsSize * sizeof(int)); | vector<int> result(nums.size(), 0); | let result = new Array(nums.length).fill(0); | result = [-1] * len(nums) | result := make([]int, n) | 

* C++ 中 `sizeof(array)` 是用来求对象所占内存空间的大小。

## 记录

* 数组问题为了减小时间复杂度，考虑双指针(索引)。
